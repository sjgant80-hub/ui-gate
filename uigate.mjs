...
function deadControls(html) {
  ...
  const whole = S(html);
  // Definitions are searched in the WHOLE document (robust — never loses a real definition, which is
  // what a per-script split does on unclosed single-file pages). Reachability is decided separately:
  // an inline onclick runs in GLOBAL scope, so a fn is reachable iff a definition sits OUTSIDE every
  // <script type="module"> block, or it is assigned to window/globalThis. A fn defined ONLY inside a
  // module block is module-scoped — "defined" but unreachable. So we need the module byte-ranges only.
  const moduleRanges = [];
  const openRe = /<script\b([^>]*)>/gi; let om;
  while ((om = openRe.exec(whole))) {
    const start = openRe.lastIndex;
    let end = whole.indexOf('</script', start); if (end === -1) end = whole.length;
    if (/\btype\s*=\s*["']?module/i.test(om[1])) moduleRanges.push([start, end]);
    openRe.lastIndex = end;
  }
  const inModule = (pos) => moduleRanges.some(([s, e]) => pos >= s && pos < e);
  ...
  for (const fn of handlers) {
    ...
    let anyDef = false, reachableDef = false, dm; def.lastIndex = 0;
    while ((dm = def.exec(whole))) { anyDef = true; if (!inModule(dm.index)) { reachableDef = true; break; } }
    if (onWindow || reachableDef) continue;                        // global — reachable from inline onclick
    if (anyDef) {
...
// A page can define its palette in an EXTERNAL stylesheet (<link rel=stylesheet> / @import). This static
// scan reads only the HTML — it cannot see that CSS — so a var used here but not defined here is NOT a
// confirmed defect when such a stylesheet is linked (the definition may live in the file we didn't read).
// Font stylesheets (Google Fonts) define no colour vars, so they don't count as an unread palette source;
// a data: stylesheet is inline, so it is not "unread". Blind-flagging externally-defined vars is exactly
// the false-positive class that inflated the 2026-09-25 estate pass ~3× (fallmarket/fallforge et al. were
// FINE — their palettes live in assets/*.css) — and a gate that cries wolf is worse than none.
function linksUnreadStylesheet(html) {
  const linkRe = /<link\b[^>]*>/gi; let m;
  while ((m = linkRe.exec(html))) {
    const tag = m[0];
    if (!/\brel\s*=\s*["']?\s*stylesheet\b/i.test(tag)) continue;
    const href = (tag.match(/\bhref\s*=\s*["']([^"']*)["']/i) || [])[1] || '';
    if (!href || /^data:/i.test(href)) continue;                          // inline / no target: nothing unread
    if (/fonts\.googleapis\.com|fonts\.gstatic\.com/i.test(href)) continue; // font CSS: no colour vars
    return true;                                                          // a real external stylesheet we can't read
  }
  // @import of a non-font, non-data target pulls external CSS too
  const impRe = /@import\s+(?:url\(\s*)?["']?([^"')\s]+)/gi;
  while ((m = impRe.exec(html))) {
    const url = m[1] || '';
    if (/^data:/i.test(url) || /fonts\.googleapis\.com|fonts\.gstatic\.com/i.test(url)) continue;
    return true;
  }
  return false;
}
function cssVars(html) {
  ...
  // an unread external stylesheet may define them — downgrade to ONE low caveat, never claim a defect.
  if (linksUnreadStylesheet(html)) {
    return [{ kind: 'external-css-caveat', severity: 'low',
      why: `${unresolved.length} CSS var(s) are used but not defined in this HTML (${unresolved.join(', ')}) — they may be defined in a linked stylesheet this static scan did not read; verify in a browser`,
      evidence: unresolved.slice(0, 8).map((v) => `var(${v})`).join(' ') }];
  }
  // inline-only page: a used-but-undefined var genuinely falls back to nothing.
  return unresolved.map((v) => ({ kind: 'undefined-css-var', severity: 'medium', why: `CSS var ${v} is used but never defined — it silently falls back to nothing`, evidence: `var(${v})` }));
}

// ── 5 · placeholder / dead links ──────────────────────────────────────────────────────────────────
// Scan the markup with <script>/<style> bodies removed FIRST: a JS template string like
// `el.innerHTML = '<a href="#">' + esc(f) + '</a>'` is code, not a rendered link — matching it flagged
// 50 phantom dead-links on fallharbor in the 2026-09-25 pass. <a> tags never live in <style>. Dynamically
// injected links are then invisible to this static pass — the honest first-pass tradeoff (no cry-wolf).
function markupNoScripts(html) {
  return S(html)
    .replace(/<script\b[^>]*>[\s\S]*?<\/script\s*>/gi, ' ')
    .replace(/<script\b[^>]*>[\s\S]*$/i, ' ')   // unclosed trailing script → strip to EOF
    .replace(/<style\b[^>]*>[\s\S]*?<\/style\s*>/gi, ' ');
}
function deadLinks(html) {
  const out = []; let m;
  const src = markupNoScripts(html);
